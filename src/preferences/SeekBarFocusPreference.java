...
public class SeekBarFocusPreference extends DialogPreference implements SeekBar.OnSeekBarChangeListener, OnClickListener
{
	...
	public SeekBarFocusPreference(Context context, AttributeSet attrs) {
		super(context,attrs);
	...
	public CharSequence getSummary() {
		float val = 0.0f;
		try {val = Float.parseFloat(getPersistedString(Float.toString(defValue)));}
		catch(NumberFormatException e) {val = defValue;}

		String v = StringUtils.getFocusDistanceString(val);
	...
	{
		String v = StringUtils.getFocusDistanceString(new BigDecimal((float)value*step+minValue).setScale(4, BigDecimal.ROUND_HALF_UP).floatValue());
