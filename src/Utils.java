...
public class Utils {
	...
	public static String getStringResourceByName(String prefix, String string) {
		int resId = resources.getIdentifier(prefix.concat(string.replace("-", "_")), "string", MainActivity.RESOURCES_PACKAGE_NAME);
		if (resId == 0) {
			return string;
		} else {
	...
	public static ArrayList<String> readConfigFile(String file) {
		if( MyDebug.LOG )
			Log.d(TAG, "readConfigFile(" + file + ")");
	...
	public static float[][] readToneMappingFile(String file) {
		ArrayList<String> lines = Utils.readConfigFile(file);
		if (lines != null && lines.size() > 0) {
			int levelsSize = lines.size();
			boolean success = true;

			float[][] levels = new float[3][levelsSize*2];
			...
			try {
				for (int i = 0; i < levelsSize; i++) {
					...
					switch (cur.length) {
						case 1: {
							float levelIn = ((float)i) / (float)(levelsSize-1);
							float levelOut = Float.parseFloat(cur[0]);

							levels[0][i*2] = levelIn;
							levels[0][i*2+1] = levelOut;

							levels[1][i*2] = levelIn;
							levels[1][i*2+1] = levelOut;

							levels[2][i*2] = levelIn;
							levels[2][i*2+1] = levelOut;

							break;
						}
						case 2: {
							float levelIn = Float.parseFloat(cur[0]);
							float levelOut = Float.parseFloat(cur[1]);
						...
						case 3: {
							...
							levels[0][i*2] = levelIn;
							levels[0][i*2+1] = Float.parseFloat(cur[0]);

							levels[1][i*2] = levelIn;
							levels[1][i*2+1] = Float.parseFloat(cur[1]);

							levels[2][i*2] = levelIn;
							levels[2][i*2+1] = Float.parseFloat(cur[2]);

							break;
						}
						case 6: {
							levels[0][i*2] = Float.parseFloat(cur[0]);
							levels[0][i*2+1] = Float.parseFloat(cur[1]);

							levels[1][i*2] = Float.parseFloat(cur[2]);
							levels[1][i*2+1] = Float.parseFloat(cur[3]);

							levels[2][i*2] = Float.parseFloat(cur[4]);
							levels[2][i*2+1] = Float.parseFloat(cur[5]);
	...
	public static boolean readBadPixelsFile(String file, HashSet<Point> badPixels, HashSet<Rect> badPixelBlocks) {
			...
			for (String line : lines) {
				...
				try {
					if (cur.length == 2) {
						...
						badPixels.add(new Point(Math.max(x ,0), Math.max(y, 0)));
					} else if (cur.length == 4) {
						int left = Integer.parseInt(cur[0]);
						int top = Integer.parseInt(cur[1]);
						int right = Integer.parseInt(cur[2]);
						int bottom = Integer.parseInt(cur[3]);

						badPixelBlocks.add(new Rect(Math.max(left ,0), Math.max(top, 0), Math.max(right ,0), Math.max(bottom, 0)));
					}
				} catch (NumberFormatException e) {}
			}
		}
