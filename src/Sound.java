...
public class Sound {
	...
	private static boolean can_use_separated_shutter = false;
	private static int sound_shutter_open = 0;
	private static int sound_shutter_close = 0;
	private static boolean shutter_open_sound_played = false;
	...
	public static void init(Context c, boolean camera_2) {
		...
		if( sound_pool == null ) {
			...
			if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ) {
				...
				sound_pool = new SoundPool(2, audio_stream, 0);
			}
			...
			can_use_separated_shutter = false;
			if (soundShutter == ShutterSound.CUSTOM) {
				if (shutter_sound.equals("separated")) {
					int resource_id = context.getResources().getIdentifier("shutter", "raw", MainActivity.RESOURCES_PACKAGE_NAME);
					
					if (resource_id != 0)
						sound_shutter = sound_pool.load(context, resource_id, 1);

					if( MyDebug.LOG )
						Log.d(TAG, "shutter resource_id = "+resource_id);

					can_use_separated_shutter = true;

					resource_id = context.getResources().getIdentifier("shutter_open", "raw", MainActivity.RESOURCES_PACKAGE_NAME);
					
					if( MyDebug.LOG )
						Log.d(TAG, "shutter_open resource_id = "+resource_id);

					if (resource_id != 0) {
						sound_shutter_open = sound_pool.load(context, resource_id, 1);

						resource_id = context.getResources().getIdentifier("shutter_close", "raw", MainActivity.RESOURCES_PACKAGE_NAME);
						
						if( MyDebug.LOG )
							Log.d(TAG, "shutter_close resource_id = "+resource_id);

						if (resource_id != 0)
							sound_shutter_close = sound_pool.load(context, resource_id, 1);
						else
							can_use_separated_shutter = false;
					} else
						can_use_separated_shutter = false;
				} else {
					int resource_id = context.getResources().getIdentifier(shutter_sound, "raw", MainActivity.RESOURCES_PACKAGE_NAME);
	...
	public static void release() {
		...
		if( sound_pool != null ) {
			...
			sound_shutter = 0;
			can_use_separated_shutter = false;
			sound_shutter_open = 0;
			sound_shutter_close = 0;
		}
	...
	public static void playShutterSound() {
		...
		shutter_open_sound_played = false;
	}

	public static boolean canUseSeparatedShutterSound() {
		return can_use_separated_shutter;
	}

	public static void playShutterSound(final long exposure) {
		if( MyDebug.LOG )
			Log.d(TAG, "playShutterOpenSound()");
			
		shutter_open_sound_played = false;

		if (shutterSoundAllowed && soundShutter == ShutterSound.CUSTOM && sound_shutter != 0) {
			if (exposure >= 500L) {
				doPlaySound(sound_shutter_open);
				shutter_open_sound_played = true;
			} else if (exposure <= 20L) {
				doPlaySound(sound_shutter);
			} else {
				new Thread(new Runnable() {
					public void run() {
						doPlaySound(sound_shutter_open);
						try {
							Thread.sleep(Math.max((int)exposure, 10));
						} catch(InterruptedException e) {}
						doPlaySound(sound_shutter_close);
					}
				}).start();
			}
		}
	...
	public static void playShutterCloseSound() {
		if( MyDebug.LOG )
			Log.d(TAG, "playShutterCloseSound()");

		if (shutter_open_sound_played && shutterSoundAllowed && soundShutter == ShutterSound.CUSTOM && sound_shutter_close != 0)
			doPlaySound(sound_shutter_close);
