...
public class PopupView extends LinearLayout {
	...
	public PopupView(Context context, PopupType popup_type) {
		...
		if (popup_type == PopupType.Flash) {
					...
					public void onClick(String selected_value) {
						if( preview.getCameraController() != null ) {
							if( preview.getCameraController().sceneModeAffectsFunctionality() ) {
								...
								main_activity.updateForSettings(getResources().getString(R.string.scene_mode) + ": " + Utils.getStringResourceByName(selected_value.equals("auto") ? "" : "sm_", selected_value));
		...
		} else if (popup_type == PopupType.Camera) {
			addRadioOptionsToPopup(preview.getCameraControllerManager().getCamerasList(), 
					getResources().getString(R.string.camera),
					Prefs.getCameraIdPref(),
					new RadioOptionsListener() {
				@Override
				public void onClick(int selected_value) {
					main_activity.setCamera(selected_value);
				}
			});
		} else {
			if (!main_activity.getMainUI().isVisible(R.id.switch_camera) && Prefs.getBoolean(Prefs.POPUP_CAMERA, false)) {
				addExpandableRadioOptionsToPopup(preview.getCameraControllerManager().getCamerasList(), 
						getResources().getString(R.string.camera),
						Prefs.getCameraIdPref(),
						new RadioOptionsListener() {
					@Override
			...
			if (!main_activity.getPreview().isVideoRecording()) {
				...
				if (preview.supportsApertures() && preview.isManualMode() && Prefs.getBoolean(Prefs.POPUP_APERTURE, true)) {
					List<Float> apertures = preview.getSupportedApertures();
					List<String> entries = new ArrayList<>();
					for (float aperture : apertures) {
						entries.add("f/" + Float.toString(aperture));
					}
					addArrayOptionsToPopup(
						getResources().getString(R.string.aperture),
						false,
						entries,
						apertures,
						Prefs.APERTURE + "_" + camera_id,
						apertures.get(0),
						false, 
						new ArrayOptionsPopupListener() {
							@Override
							public void onValueChanged(float value) {
								preview.getCameraController().setAperture(value);
							}
						}
					);
				}

				if( Prefs.getBoolean(Prefs.POPUP_AUTO_LEVEL, true) && (preview.isVideo() ? preview.supportsVideoStabilization() : main_activity.supportsAutoLevel()) ) {
					addCheckBoxToPopup(
						getResources().getString(preview.isVideo() ? R.string.preference_video_stabilization : R.string.preference_auto_level),
						preview.isVideo() ? Prefs.VIDEO_STABILIZATION : Prefs.AUTO_LEVEL, false,
						new CheckBoxPopupListener() {
							@Override
							public void onCheckedChanged(boolean isChecked) {
								boolean done_dialog = false;
								if( isChecked ) {
									boolean done_auto_level_info = Prefs.contains(preview.isVideo() ? Prefs.DONE_VIDEO_STABILIZATION_INFO : Prefs.DONE_AUTO_LEVEL_INFO);
									if( !done_auto_level_info ) {
										...
										else showInfoDialog(R.string.preference_auto_level, R.string.auto_level_info, Prefs.DONE_AUTO_LEVEL_INFO);
										done_dialog = true;
									}
								}

								if( !done_dialog ) {
									String message = getResources().getString(preview.isVideo() ? R.string.preference_video_stabilization : R.string.preference_auto_level) + ": " + getResources().getString(isChecked ? R.string.on : R.string.off);
									Utils.showToast(main_activity.getChangedAutoLevelToastBoxer(), message);
								}
							}
						}
					);
				...
				if( Prefs.getBoolean(Prefs.POPUP_STAMP, false)) {
					addCheckBoxToPopup(
						getResources().getString(R.string.preference_stamp),
						Prefs.STAMP, false, null
					);
				}
				...
				if (!preview.isVideo() && Prefs.getBoolean(Prefs.POPUP_RAW, false)) {
					addCheckBoxToPopup(
						getResources().getString(R.string.preference_raw),
						Prefs.RAW, false,
						new CheckBoxPopupListener() {
							@Override
							public void onCheckedChanged(boolean isChecked) {
								if( isChecked ) {
									if( !Prefs.contains(Prefs.DONE_RAW_INFO) ) {
										showInfoDialog(R.string.preference_raw, R.string.raw_info, Prefs.DONE_RAW_INFO);
									}
								}
				...
				if( Prefs.getBoolean(Prefs.POPUP_HISTOGRAM, false)) {
					addCheckBoxToPopup(
						...
						Prefs.SHOW_HISTOGRAM, false, null
					);
				}
				...
				if (!preview.isVideo()) {
					final Prefs.PhotoMode photo_mode = Prefs.getPhotoMode();
					if (photo_mode == Prefs.PhotoMode.ExpoBracketing && Prefs.getBoolean(Prefs.POPUP_PHOTOS_COUNT, true)) {
						final List<String> entries = Arrays.asList(getResources().getStringArray(R.array.preference_expo_bracketing_images_entries));
						final List<String> values = Arrays.asList(getResources().getStringArray(R.array.preference_expo_bracketing_images_values));

						addArrayOptionsToPopup(
							getResources().getString(R.string.preference_expo_bracketing_darker_images),
							false,
							entries,
							values,
							Prefs.EXPO_BRACKETING_DOWN,
							"1",
							false, 
							new ArrayOptionsPopupListener() {
								@Override
						...
						addArrayOptionsToPopup(
							getResources().getString(R.string.preference_expo_bracketing_lighter_images),
							false,
							entries,
							values,
							Prefs.EXPO_BRACKETING_UP,
							"1",
							false, 
							new ArrayOptionsPopupListener() {
								@Override
					...
					if ((photo_mode == Prefs.PhotoMode.ExpoBracketing || photo_mode == Prefs.PhotoMode.HDR) && Prefs.getBoolean(Prefs.POPUP_EXPO_BRACKETING_STOPS, true)) {
						final List<String> entries = Arrays.asList(getResources().getStringArray(R.array.preference_expo_bracketing_stops_entries));
						...
						if (photo_mode == Prefs.PhotoMode.ExpoBracketing) {
							addArrayOptionsToPopup(
								getResources().getString(R.string.preference_expo_bracketing_stops),
								false,
								entries,
								values,
								Prefs.EXPO_BRACKETING_STOPS,
								"2",
								false, 
								new ArrayOptionsPopupListener() {
									@Override
...
/*								new CheckBoxPopupListener() {
	...
	public void switchToWhiteBalance(String selected_value) {
		...
		if( preview.getCameraController() != null ) {
			if (
					selected_value.equals("manual") ||
					preview.getCameraController().getWhiteBalance().equals("manual") ||
					selected_value.equals("rgb") ||
					preview.getCameraController().getWhiteBalance().equals("rgb")
				) {
				update_seekbars = true;
			}
			preview.getCameraController().setWhiteBalance(selected_value);
			if( selected_value.equals("manual") || selected_value.equals("rgb") ) {
		...
		if( options != null ) {
			...
			for(final String supported_option : supported_options) {
				...
				if( icons != null && values != null ) {
					...
					if( index != -1 ) {
						resource = getResources().getIdentifier(icons[index], null, MainActivity.RESOURCES_PACKAGE_NAME);
					}
				}
				if( MyDebug.LOG )
				...
				if( prefix_string.length() == 0 ) {
					...
					if (supported_option.length() > 3 && supported_option.substring(0, 3).equals("le_"))
	...
	private void addRadioOptionsToGroup(
		...
		for(final String supported_option : supported_options) {
			...
			if (supported_option.equals("auto")) translated_text = getResources().getString(R.string.auto);
				...
				if (supported_option.equals("manual"))
					translated_text = getResources().getString(R.string.iso_manual);
				else if (supported_option.length() > 3 && supported_option.substring(0, 3).equals("le_"))
					translated_text = "\u2264 " + supported_option.substring(3);
				else 
					translated_text = StringUtils.fixISOString(supported_option);
	...
	private void addRadioOptionsToPopup(
		final String[] supported_options,
		final String title,
		final int current_option,
		final RadioOptionsListener listener
	) {
		if( MyDebug.LOG )
			Log.d(TAG, "addRadioOptionsToPopup: " + title);
		if( supported_options != null ) {
			...
			final RadioGroup rg = new RadioGroup(this.getContext());
			rg.setOrientation(RadioGroup.VERTICAL);
			addRadioOptionsToGroup(rg, supported_options, current_option, listener);
			this.addView(rg);
		}
	}
	...
	private void addRadioOptionsToGroup(
		...
		if( MyDebug.LOG )
			Log.d(TAG, "addRadioOptionsToGroup");
		final long debug_time = System.nanoTime();
		final MainActivity main_activity = (MainActivity)this.getContext();
		for(int i = 0; i < supported_options.length; i++) {
			final int supported_option = i;
			if( MyDebug.LOG ) {
				Log.d(TAG, "supported_option: " + supported_option);
			}
			RadioButton button = new RadioButton(this.getContext());
			...
			button.setButtonDrawable(negative ? R.drawable.radio_selector_dark : R.drawable.radio_selector_light);
			button.setId(supported_option);

			button.setText(supported_options[supported_option]);
			button.setTextSize(TypedValue.COMPLEX_UNIT_PX, text_size_main);
			button.setTextColor(negative ? Color.BLACK : Color.WHITE);
			rg.addView(button);

			if(supported_option == current_option) {
				rg.check(supported_option);
			}

			button.setContentDescription(supported_options[supported_option]);
			button.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					if( listener != null ) {
	...
	private class ArrayOptions extends LinearLayout {
		...
		public ArrayOptions(
			...
			this.entries = entries;
			this.values_float = values;
			this.pref_key = pref_key;
			float value = Prefs.getFloat(pref_key, default_value);
			current_index = values_float.indexOf(value);
			if( current_index == -1 ) {
				if( MyDebug.LOG )
					Log.d(TAG, "can't find value " + value + " in values!");
				current_index = values_float.indexOf(default_value);
				if( current_index == -1 )
					current_index = 0;
			}
			this.values_count = values_float.size();
		...
		private void valueChanged() {
			if (values_float == null) {
				...
				float value = values_float.get(current_index);
				Prefs.setFloat(pref_key, value);
				listener.onValueChanged(value);
			}
		}
		...
		private void updateText() {
			if (entries != null) {
				...
				if (max_buttons_count >= 8)
					text_view.setText(size.width + " x " + size.height + " " + StringUtils.getAspectRatioMPString(size.width, size.height));
				else if (max_buttons_count >= 6)
					text_view.setText(size.width + " x " + size.height + " (" + StringUtils.getMPString(size.width, size.height) + ")");
				else
	...
	private List<String> getSupportedISOs(final MainActivity main_activity) {
		if( main_activity.getPreview().supportsISORange() ) {
			...
			if (!main_activity.getPreview().isVideo()) {
				boolean values_fixed = Prefs.getBoolean(Prefs.POPUP_ISO_FIXED, true);
				boolean values_max = Prefs.getBoolean(Prefs.POPUP_ISO_MAX, true);
				if (values_fixed || values_max)
					values.add("" + min_iso);
				...
				if (values_max) {
					for(int iso_value : new int[]{50, 100, 200, 400, 800, 1600, 3200, 6400}) {
						if( iso_value > min_iso && iso_value < max_iso ) {
							values.add("le_" + iso_value);
						}
					}
					values.add("le_" + max_iso);
				}
			}
			return values;
		}
