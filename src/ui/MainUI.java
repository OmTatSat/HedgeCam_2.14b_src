...
public class MainUI {
	...
	private int align_top = RelativeLayout.ALIGN_TOP;
	...
	public final int[] manual_control = {
		...
		R.id.white_balance_seekbar_b,
		R.id.white_balance_seekbar_g,
		R.id.white_balance_seekbar_r,
	...
	private final int[] seekbar_icons = {
		...
		R.id.white_balance_seekbar_r_icon,
		R.id.white_balance_seekbar_g_icon,
		R.id.white_balance_seekbar_b_icon,
	};
	...
	public void layoutUI() {
		...
		if (system_ui_portrait) {
			...
			if( ui_placement_right ) {
				...
				align_top = RelativeLayout.ALIGN_RIGHT;
			} else {
				...
				align_top = RelativeLayout.ALIGN_LEFT;
			}
		} else {
			...
			if( ui_placement_right ) {
				...
				align_top = RelativeLayout.ALIGN_TOP;
			} else {
...
//			}
			...
			if (preview_has_gap && preview.hasAspectRatio()) {
				double preview_ar = preview.getAspectRatio();
			...
			if (buttons_count !=0 ) {
				...
				if (gui_type == GUIType.Phone || gui_type == GUIType.Phone2){
					...
					if (gui_type == GUIType.Tablet) {
						...
						if (main_activity.supportsVideoPause() && preview.isVideo()) hide_buttons = new ArrayList<>();
					} else {
					...
					for(int i = 0; i < ctrl_panel_buttons.length; i++) {
						...
						if (view.getVisibility() == View.VISIBLE && buttons_location[i] != 2) {
							...
							if (main_activity.supportsVideoPause() && gui_type == GUIType.Tablet && preview.isVideo() && angle > 150) {
					...
					if( gui_type == GUIType.Tablet && (ui_rotation_relative == 0 || ui_rotation_relative == 180) ) ind_margin_right = gallery_width+gallery_margin;
				}
			}
	...
	private void layoutPopupView(boolean only_params) {
		...
		if (from_mode_panel) {
		...
		} else if (gui_type == GUIType.Tablet || gui_type == GUIType.Universal || popup_from == R.id.switch_camera) {
			layoutParams.addRule(center_horizontal, 0);
			layoutParams.addRule(center_vertical, RelativeLayout.TRUE);
			layoutParams.addRule(align_parent_top, 0);
			layoutParams.addRule(align_parent_bottom, 0);
			...
			layoutParams.addRule(above, 0);
			layoutParams.addRule(below, 0);
			layoutParams.addRule(left_of, gui_type == GUIType.Phone ? 0 : popup_anchor);
			layoutParams.addRule(right_of, gui_type == GUIType.Phone ? popup_anchor : 0);
			if (system_ui_portrait) {
		...
		if (ui_rotation_portrait) {
			if (from_mode_panel) {
				...
				if (popup_from != R.id.switch_camera)
	...
	public void setTakePhotoIcon() {
		...
		if( preview != null ) {
			ImageButton view;
			final boolean is_video = preview.isVideo();
			int resource = 0;
			int bg_resource = 0;
			...
			if (
				preview.isOnTimer()
				|| preview.isBurst()
				|| preview.isWaitingFace()
				|| preview.isVideoRecording()
			) {
	...
	public void setSwitchCameraContentDescription() {
		...
		if( preview != null && preview.canSwitchCamera() ) {
	...
	public void setImmersiveMode(final boolean immersive_mode) {
		setImmersiveMode(immersive_mode, false);
	}

	public void setImmersiveMode(final boolean immersive_mode, final boolean hide_all) {
		if( MyDebug.LOG )
			Log.d(TAG, "setImmersiveMode: " + immersive_mode);
		if (this.immersive_mode != immersive_mode) {
			if (immersive_mode && popupIsOpen()) {
				closePopup();
			}
			this.immersive_mode = immersive_mode;
			main_activity.runOnUiThread(new Runnable() {
				public void run() {
					if (Prefs.getString(Prefs.IMMERSIVE_MODE, "immersive_mode_off").equals("immersive_mode_overlay")) {
						if (immersive_mode) {
							main_activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
							main_activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_IMMERSIVE);
						} else {
							main_activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
							main_activity.getWindow().getDecorView().setSystemUiVisibility(0);
						}
					}
					showGUI(!immersive_mode, Prefs.getString(Prefs.IMMERSIVE_MODE, "immersive_mode_off").equals("immersive_mode_everything") || hide_all);
				}
			});
		}
	}
		...
		main_activity.runOnUiThread(new Runnable() {
			public void run() {
				...
				main_activity.findViewById(R.id.zoom)
					.setVisibility(( preview.supportsZoom() && Prefs.getBoolean(Prefs.SHOW_ZOOM_CONTROLS, false) ) ? visibility : View.GONE);

				main_activity.findViewById(R.id.zoom_seekbar)
					.setVisibility(( preview.supportsZoom() && Prefs.getBoolean(Prefs.SHOW_ZOOM_SLIDER_CONTROLS, false) ) ? visibility : View.GONE);

				main_activity.findViewById(R.id.take_photo)
					.setVisibility((Prefs.getBoolean(Prefs.SHOW_TAKE_PHOTO, true)) ? visibility : View.GONE);
					
				if (!show || show_seekbars)
					showSeekbars(show, false);
				setManualFocusSeekbars();
	...
	private void updateButtonsLocation() {
		...
		buttons_location[BUTTON_IMMERSIVE_MODE] = 0;
		if (m && Prefs.getBoolean(Prefs.MODE_PANEL_IMMERSIVE_MODE, false)) buttons_location[BUTTON_IMMERSIVE_MODE] = 2;
		else if (Prefs.getBoolean(Prefs.CTRL_PANEL_IMMERSIVE_MODE, false)) buttons_location[BUTTON_IMMERSIVE_MODE] = 1;

		buttons_location[BUTTON_FOCUS_MODE] = 0;
		if (preview.supportsFocus() && (preview.isVideo() || Prefs.getPhotoMode() != Prefs.PhotoMode.FocusBracketing)) {
		...
		if (!preview.isVideo() && (main_activity.supportsDRO() || main_activity.supportsHDR() || main_activity.supportsExpoBracketing())) {
		...
		if (preview.getMaxNumMeteringAreas() > 0) {
		...
		buttons_location[BUTTON_AUTO_WHITE_BALANCE_LOCK] = 0;
		if (preview.supportsAutoAdjustmentLock()) {
			if (m && Prefs.getBoolean(Prefs.MODE_PANEL_LOCK_WB, false)) buttons_location[BUTTON_AUTO_WHITE_BALANCE_LOCK] = 2;
			else if (Prefs.getBoolean(Prefs.CTRL_PANEL_LOCK_WB, false)) buttons_location[BUTTON_AUTO_WHITE_BALANCE_LOCK] = 1;
		}
		...
		if (preview.getCameraControllerManager().getNumberOfCameras() > 1) {
	...
	public void enableClickableControls(final boolean enable) {
		main_activity.runOnUiThread(new Runnable() {
			public void run() {
				...
				view.setAlpha(alpha);
				boolean is_video = preview.isVideo();
				for(int id : ctrl_panel_buttons) {
					view = (View) main_activity.findViewById(id);
					if (view.getVisibility() == View.VISIBLE && (enable || (!is_video || (id != R.id.exposure && id != R.id.auto_adjustment_lock && id != R.id.auto_wb_lock)))){
						view.setEnabled(enable);
	...
	public void showSeekbars(boolean show, boolean layout) {
		if( MyDebug.LOG )
			Log.d(TAG, "showSeekbars(" + show + ", " + layout + ")");
		...
		if (show) {
			if( preview.getCameraController() != null ) {
				...
				if( preview.supportsISORange() && iso_value.equals("manual") ) {
					seekbar_iso = true;
					seekbar_exposure_time = preview.supportsExposureTime();
				}
				else {
					seekbar_exposure = preview.supportsExposures();
					seekbar_exposure_buttons = seekbar_exposure && Prefs.getBoolean(Prefs.SHOW_EXPOSURE_BUTTONS, true);
				}

				if( preview.usingCamera2API() && preview.supportsWhiteBalanceTemperature()) {
					// we also show slider for manual white balance, if in that mode
					String wb_value = Prefs.getString(Prefs.WHITE_BALANCE, "auto");
					seekbar_wb = wb_value.equals("manual");
					seekbar_rgb = wb_value.equals("rgb");
				}
			}
		}

		main_activity.findViewById(R.id.iso_seekbar).setVisibility(seekbar_iso ? View.VISIBLE : View.GONE);

		main_activity.findViewById(R.id.exposure_time_seekbar).setVisibility(seekbar_exposure_time ? View.VISIBLE : View.GONE);

		main_activity.findViewById(R.id.white_balance_seekbar).setVisibility(seekbar_wb ? View.VISIBLE : View.GONE);

		main_activity.findViewById(R.id.white_balance_seekbar_r).setVisibility(seekbar_rgb ? View.VISIBLE : View.GONE);
		main_activity.findViewById(R.id.white_balance_seekbar_g).setVisibility(seekbar_rgb ? View.VISIBLE : View.GONE);
		main_activity.findViewById(R.id.white_balance_seekbar_b).setVisibility(seekbar_rgb ? View.VISIBLE : View.GONE);

		main_activity.findViewById(R.id.exposure_seekbar).setVisibility(seekbar_exposure ? View.VISIBLE : View.GONE);

		main_activity.findViewById(R.id.exposure_seekbar_zoom).setVisibility(seekbar_exposure_buttons ? View.VISIBLE : View.GONE);

		if (layout) layoutSeekbars();
	}

	public void updateSeekbars() {
		if (Prefs.getBoolean(Prefs.SLIDERS_AUTO_SWITCH, true)) {
			String wb_value = Prefs.getString(Prefs.WHITE_BALANCE, "auto");
			show_seekbars = Prefs.getISOPref().equals("manual") || wb_value.equals("manual") || wb_value.equals("rgb");
			Prefs.setBoolean(Prefs.SHOW_SEEKBARS, show_seekbars);
			showSeekbars(show_seekbars, true);
	...
	public void setFlashIcon() {
		...
		setPopupIcon(R.id.flash_mode, R.array.flash_values, R.array.flash_icons,
				preview.getCurrentFlashValue(), R.drawable.ctrl_flash_on);
	}
	...
	public void setFocusIcon() {
		...
		setPopupIcon(R.id.focus_mode, R.array.focus_mode_values, R.array.focus_mode_icons,
				preview.getCurrentFocusValue(), R.drawable.ctrl_focus_mode);
	}
	...
	public void setPopupIcon(final int icon_id, final int values_id, final int icons_id, final String current_value, final int default_icon) {
		...
		if( icons != null && values != null ) {
			...
			if( index != -1 ) {
				resource = resources.getIdentifier(icons[index], null, MainActivity.RESOURCES_PACKAGE_NAME);
			}
		}
	...
	public void setExposureIcon() {
		...
		if (!Prefs.getISOPref().equals("manual")) {
			int value = preview.getCurrentExposure();
			if (value > 0) {
	...
	public void setISOIcon() {
		...
		if (value.equals("auto"))
			...
			text = "M";
		else if (value.length() > 3 && value.substring(0, 3).equals("le_"))
	...
	public void setPopupIcons() {
		...
		boolean auto_wb_lock_visible = isVisible(R.id.auto_wb_lock);
		if (auto_wb_lock_visible) {
			ImageButton button = (ImageButton)main_activity.findViewById(R.id.auto_wb_lock);
			button.setImageResource(preview.isAutoWhiteBalanceLocked() ? R.drawable.ctrl_lock_wb_red : R.drawable.ctrl_lock_wb);
		}
		if (isVisible(R.id.auto_adjustment_lock)) {
			boolean locked = auto_wb_lock_visible ? preview.isAutoExposureLocked() : preview.isAutoAdjustmentLocked();
			ImageButton button = (ImageButton)main_activity.findViewById(R.id.auto_adjustment_lock);
			button.setImageResource(locked ? R.drawable.ctrl_lock_red : R.drawable.ctrl_lock);
		}
	}
	...
	public void togglePopup(View view) {
		...
		switch (view.getId()) {
			...
			case R.id.switch_camera:
				popup_type = PopupView.PopupType.Camera;
				break;
		}
		...
		if (popup_type == PopupView.PopupType.ISO && preview.supportsISORange() && preview.isVideo()) {
			...
			return;
		} else if (popup_type == PopupView.PopupType.Flash && preview.isVideo()) {
	...
	private void saveZoom() {
		if (preview.supportsZoom() && Prefs.getBoolean(Prefs.SAVE_ZOOM, false)) {
			...
			if (camera_controller != null) {
				String pref_key = Prefs.ZOOM + (preview.usingCamera2API() ? "_2_" : "_1_") + camera_controller.getCameraId();
	...
	private final int[] std_exposures = {
		64000, 50000, 32000, 25000, 16000, 10000, 8000, 5000, 4000, 2000, 1000,
		800, 500, 400, 250, 200, 125, 100, 80, 60, 50, 40, 30, 25, 20, 15, 12, 10,
		8, 5, 4, 2, 1, 2, 4, 5, 8, 10, 12, 15, 20, 25, 30, 35, 40, 50, 60, 80, 100
	};
	...
	public void setManualFocusSeekbars() {
		...
		final boolean is_bracketing = show_gui && (!preview.isVideo() && Prefs.getPhotoMode() == Prefs.PhotoMode.FocusBracketing);
		...
		final boolean is_visible = show_gui && (is_bracketing || (preview.getCurrentFocusValue() != null && preview.getCurrentFocusValue().equals("focus_mode_manual2")));
		if (is_visible) {
	...
	public void setManualWBSeekbar() {
		...
		if( preview.getSupportedWhiteBalances() != null && preview.supportsWhiteBalanceTemperature() ) {
			...
			} else if (wb_value.equals("rgb")) {
				final int rgb_min = 50;
				final int rgb_max = 150;
				
				final int r = Prefs.getInt(Prefs.WHITE_BALANCE_R, 100);
				final int g = Prefs.getInt(Prefs.WHITE_BALANCE_G, 100);
				final int b = Prefs.getInt(Prefs.WHITE_BALANCE_B, 100);
				preview.setWhiteBalanceRGB(r, g, b);

				final SeekBar seek_bar_r = ((SeekBar)main_activity.findViewById(R.id.white_balance_seekbar_r));
				final SeekBar seek_bar_g = ((SeekBar)main_activity.findViewById(R.id.white_balance_seekbar_g));
				final SeekBar seek_bar_b = ((SeekBar)main_activity.findViewById(R.id.white_balance_seekbar_b));

				seek_bar_r.setMax(rgb_max - rgb_min);
				seek_bar_r.setProgress(r - rgb_min);
				seek_bar_r.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
						if( MyDebug.LOG )
							Log.d(TAG, "white balance r seekbar onProgressChanged: " + progress);
						int new_value = rgb_min + progress;
						preview.setWhiteBalanceRGB(new_value, rgb_min + seek_bar_g.getProgress(), rgb_min + seek_bar_b.getProgress());

						if (fromUser)
							setSeekbarHint(seekBar, Integer.toString(new_value) + " %");
						else if (((View)seekBar).getVisibility() == View.VISIBLE)
							showSeekbarHint(seekBar, Integer.toString(new_value) + " %", false);
						else
							Utils.showToast("R: " + new_value + " %");
					}

					@Override
					public void onStartTrackingTouch(SeekBar seekBar) {
						showSeekbarHint(seekBar, Integer.toString(rgb_min + seekBar.getProgress()) + " %", true);
					}

					@Override
					public void onStopTrackingTouch(SeekBar seekBar) {
						Prefs.setInt(Prefs.WHITE_BALANCE_R, rgb_min + seekBar.getProgress());
						hideSeekbarHint();
					}
				});

				seek_bar_g.setMax(rgb_max - rgb_min);
				seek_bar_g.setProgress(g - rgb_min);
				seek_bar_g.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
						if( MyDebug.LOG )
							Log.d(TAG, "white balance g seekbar onProgressChanged: " + progress);
						int new_value = rgb_min + progress;
						preview.setWhiteBalanceRGB(rgb_min + seek_bar_r.getProgress(), new_value, rgb_min + seek_bar_b.getProgress());
					...
					@Override
					public void onStopTrackingTouch(SeekBar seekBar) {
						Prefs.setInt(Prefs.WHITE_BALANCE_G, rgb_min + seekBar.getProgress());
						hideSeekbarHint();
					}
				});

				seek_bar_b.setMax(rgb_max - rgb_min);
				seek_bar_b.setProgress(b - rgb_min);
				seek_bar_b.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					@Override
					public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
						if( MyDebug.LOG )
							Log.d(TAG, "white balance b seekbar onProgressChanged: " + progress);
						int new_value = rgb_min + progress;
						preview.setWhiteBalanceRGB(rgb_min + seek_bar_r.getProgress(), rgb_min + seek_bar_g.getProgress(), new_value);
					...
					@Override
					public void onStopTrackingTouch(SeekBar seekBar) {
						Prefs.setInt(Prefs.WHITE_BALANCE_B, rgb_min + seekBar.getProgress());
						hideSeekbarHint();
					}
				});
				...
				new AsyncTask<Void, Void, Bitmap>() {
					...
					protected Bitmap doInBackground(Void... params) {
						...
						if (source.equals("last_photo")) {
							...
							try {
								media = main_activity.getStorageUtils().getLatestMedia();
