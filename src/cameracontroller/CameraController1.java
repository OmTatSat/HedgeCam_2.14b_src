...
public class CameraController1 extends CameraController {
	private static final String TAG = "HedgeCam/CameraController1";
...
//						iso_key = null; // not supported
		...
		if( iso_key != null ){
			...
			if (values.size() == 1 && values.get(0).equals("auto")){
				values.add("50");
				values.add("100");
				values.add("200");
				values.add("400");
	...
	public void setAutoExposureLock(boolean enabled) {
		if( MyDebug.LOG )
			Log.d(TAG, "setAutoExposureLock: " + enabled);
	...
	public void setAutoWhiteBalanceLock(boolean enabled) {
		if( MyDebug.LOG )
			Log.d(TAG, "setAutoWhiteBalanceLock: " + enabled);
	...
	public boolean setFocusAndMeteringArea(List<CameraController.Area> areas, boolean focus_area, boolean metering_area) {
		...
		String focus_mode = parameters.getFocusMode();
		// getFocusMode() is documented as never returning null, however I've had null pointer exceptions reported in Google Play
		if( focus_area && parameters.getMaxNumFocusAreas() != 0 && focus_mode != null && ( focus_mode.equals(Camera.Parameters.FOCUS_MODE_AUTO) || focus_mode.equals(Camera.Parameters.FOCUS_MODE_MACRO) || focus_mode.equals(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE) || focus_mode.equals(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO) ) ) {
			parameters.setFocusAreas(camera_areas);

			// also set metering areas
			if( metering_area && parameters.getMaxNumMeteringAreas() == 0 ) {
				if( MyDebug.LOG )
		...
		else if( metering_area && parameters.getMaxNumMeteringAreas() != 0 ) {
			parameters.setMeteringAreas(camera_areas);
	...
	private void takePictureNow(final CameraController.ShutterCallback shutter, final CameraController.PictureCallback picture, final ErrorCallback error) {
		if( MyDebug.LOG )
			Log.d(TAG, "takePictureNow");

		Camera.ShutterCallback shutter_cb = shutter == null ? null : new Camera.ShutterCallback() {
		...
		try {
			camera.takePicture(shutter_cb, null, camera_jpeg);
		}
		catch(RuntimeException e) {
	...
	public void takePicture(final CameraController.ShutterCallback shutter, final CameraController.PictureCallback picture, final ErrorCallback error) {
		if( MyDebug.LOG )
		...
		if( want_expo_bracketing ) {
			...
			for (double ev : expo_bracketing_stack) {
				if (ev != 0.0d) {
					...
					int value = exposure_current+steps;
					requests.add(Math.max(Math.min(value, max_exposure), min_exposure));
					if( MyDebug.LOG ) {
						Log.d(TAG, "add burst request:");
						Log.d(TAG, "ev: " + ev);
	...
	public String getAntibanding() {
		return this.getParameters().getAntibanding();
	}
	...
	public long getApproximatelyTotalExposureTime() {
		long exposure_time = 100L;
		if (want_expo_bracketing && expo_bracketing_stack != null) {
			exposure_time = exposure_time*expo_bracketing_stack.size() + exposure_compensation_delay*(expo_bracketing_stack.size()-1);
		} else if (want_burst) {
			exposure_time = exposure_time*want_burst_count + 50L*(want_burst_count-1);
		}
